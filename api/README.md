...
5. **Une opération neuve que la PR ne sert pas encore.** Une opération déclarée sous `paths:` que
   personne ne sert fait **échouer la suite** — c'est voulu : le tableau de bord génère ses clients
   depuis ce YAML, et une opération déclarée sans implémentation y devient un appel vers un 404. Côté
   public, elle s'inscrit en `deferred` (raison **et** step) dans `internal/restapi/conformance_test.go`.
   Côté Admin, la liste d'exceptions a disparu avec step-390, qui a servi tout le contrat : la PR sert
   l'opération qu'elle déclare, ou réintroduit une liste annotée dans `internal/adminapi/contract_test.go`.
