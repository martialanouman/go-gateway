...
toute requête autorisée et non throttlée ; `internal/smpp/smpp.go` l'avoue dans le godoc de
`MessageStateUnknown` (« while the real state lookup is unimplemented »). Répondre `UNKNOWN` est légal
SMPP : c'est une promesse non tenue, pas une panne, et cette fiche ne bloque pas le go-live. step-390
(réglages de compte) déclare explicitement la résolution hors de son périmètre ; step-260g a ouvert
cette fiche pour que la dette ait un porteur.
...
- Le throttle dédié §6.22 est posé (`wiring.go:241-249`, compteur `smpp_query_throttled_total`) et
...
|---|---|---|
| `accepted` (pas encore soumis) | `ACCEPTED` (6) | vide |
| `enroute`, `rerouted` | `ENROUTE` (1) | vide |
| `delivered` | `DELIVERED` (2) | horodatage de la ligne |
| `expired` | `EXPIRED` (3) | horodatage |
| `cancelled` | `DELETED` (4) | horodatage |
| `failed` | `UNDELIVERABLE` (5) | horodatage |
| `rejected` | `REJECTED` (8) | horodatage |
...
soumis il y a 200 ms peut n'avoir que sa ligne `accepted` alors qu'il est déjà sur le fil. Répondre
`ACCEPTED` est **honnête** : c'est le dernier état durablement connu, et c'est exactement ce que
`get-message` REST répond au même instant (parité protocole). On ne lit **pas** Redis ni le pool pour
