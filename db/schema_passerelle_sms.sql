...
CREATE TABLE control_plane.gdpr_erase_jobs (
  ...
  operator     text NOT NULL,           -- auth.Fingerprint of the requesting operator's token, never the token (migration 0014)
  created_at   timestamptz NOT NULL DEFAULT now(),
...
-- action) and completed with its HTTP status after. Never a body, a query string or a token (invariant a).
...
CREATE TABLE control_plane.audit_log (
  ...
  status       smallint,                -- the HTTP status; NULL until the outcome is recorded
...
CREATE TABLE control_plane.billing_ledger (
  ...
  entry_type    text NOT NULL CHECK (entry_type IN ('reserve','capture','release','refund','topup','adjustment','mo_charge','transfer')),
