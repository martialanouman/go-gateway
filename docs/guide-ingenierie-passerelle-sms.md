...
**L0 — correspondance de numéro exact (priorité maximale, court-circuit).** Si le MSISDN de destination figure dans `exact_routes`, sa cible est utilisée immédiatement. C'est la réponse à la **portabilité des numéros** : le matching par préfixe (L2) suppose à tort que le préfixe identifie l'opérateur, faux pour un numéro porté. Un filtre de Bloom en mémoire (rafraîchi par `config-sync`) élimine sans appel réseau le trafic vers les numéros non portés, soit 70 à 90 % en marché MNP mûr (10 à 30 % de numéros portés, step-280 ; le « ~99 % » hérité d'ADR-0004 précédait tout chiffre MNP) — jamais de faux négatif, donc « absent » signifie certainement pas d'override. Un « peut-être » lit `exactroute:{msisdn}`, qui est un **cache read-through** sur `exact_routes` : le routeur le peuple depuis la table sur un miss, l'Admin API l'invalide après son commit (ADR-0015). Les deux jambes de lecture sont fail-closed en rejeu (§16). Si la cible d'un numéro exact est indisponible (connecteur désactivé, disjoncteur ouvert), on **retombe** sur L1/L2 plutôt que dead-letter.
...
Toute écriture d'ingestion vers Kafka est la limite de durabilité ; l'accusé n'a lieu qu'après validation dans `mt.inbound`. **Aucune perte après accusé** (remise au SMSC au moins une fois). L'exactement-une-fois n'est pas garanti de bout en bout (SMPP est au moins une fois) — des clés d'idempotence sont disponibles côté client, et la facturation est idempotente par `message_id`. **Un abonné peut donc recevoir deux fois le même SMS** : les causes résiduelles sont **deux**, une par étage qui publie avant de commiter — au pool, un crash entre le `submit_sm` parti et l'accusé du produce qui enregistre son issue ; au routeur, une interruption entre la publication d'un `mt.routed` et le commit de `mt.inbound`. Toutes deux bornées par la même variable à **un poll par partition et par incident**, soit ~250 `submit_sm` côté pool (ADR-0012 ; 56 KiB compressés, le nombre variant avec le taux de compression) ; côté routeur l'unité est le message, donc 1..N segments (ADR-0014). Files dead-letter pour les messages ayant épuisé leurs retries (y compris `fallback_chain` épuisé), remontées pour retraitement. Le retraitement de `mt.dead-letter` passe par `mt-replay -operator <nom>`, qui exige Kafka, ClickHouse **et** Postgres : il écrit sa ligne dans `control_plane.audit_log` avant de rejouer, et sans ligne ne rejoue rien (step-296). Le nom est déclaré, pas authentifié (`debts/rejeu-impute-a-une-identite-declaree.md`).
...
Les manifests vivent sous **`deploy/k8s`** (un fichier par service), et `deploy/README.md` porte ce qui ne se lit pas dans le YAML : le calcul de la période de grâce (`DRAIN_DELAY` + `DRAIN_BUDGET` + `SHUTDOWN_TIMEOUT`, le dernier terme venant de `DrainTracing` qui court après le superviseur — `SHUTDOWN_TIMEOUT` borne **un** composant, `DRAIN_BUDGET` leur séquence), la raison du plafond des HPA, et les trois dimensionnements laissés à step-280. Les images ne sont pas encore construites (step-270b) ; ni collecteur OTel, ni règles Alertmanager, ni Ingress ne partent de ce dépôt.
