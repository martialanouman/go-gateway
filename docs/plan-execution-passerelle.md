...
2. **`connector-pool-svc`** : ligne `status=enroute` (ou `rejected`/`failed`) après `submit_sm_resp`.
...
- `cmd/connector-pool-svc` : `bind_pool_size=1`, consomme `mt.routed`, `submit_sm` → faux SMSC, suit `submit_sm_resp`, écrit le CDR (`status=enroute`).
