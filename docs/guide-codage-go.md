...
| Redis (routage L0, numéro exact) | résolution de route | **fail-closed en rejeu** : erreur non codée → offset non commité → redélivrance. **Deux exceptions, et deux seulement** : une valeur illisible et une clé du **mauvais type** (`WRONGTYPE`) sont traitées comme un miss et guéries depuis la table durable | un numéro exact injoignable ne doit pas se dégrader en routage par défaut, qui enverrait sur le mauvais opérateur. Les deux exceptions vont dans l'autre sens : remonter l'erreur renverrait le message sur la **même clé** à chaque redélivrance, et une clé de mauvais type n'expire même pas d'elle-même — la partition reste figée jusqu'à un `DEL` manuel (step-290d). Tout autre code RESP (`LOADING`, `READONLY`, `OOM`…) reste une faute, sous test |
