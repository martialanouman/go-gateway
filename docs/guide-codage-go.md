...
**[MUST]** Les identifiants ne sont stockés qu'en hash (bcrypt/argon2 pour les mots de passe de bind et clés API). Le secret n'est retourné qu'à la création/rotation. Comparaisons en temps constant (`subtle.ConstantTimeCompare`) pour les vérifications d'auth — **une exception, écrite** : la clé API n'est jamais comparée en Go, la ligne est **cherchée par son hash** (index, plan §1.9), donc l'égalité est celle de PostgreSQL. Ce qu'une différence de temps y divulgue est un préfixe du SHA-256 d'une clé de 256 bits qu'aucun attaquant ne sait choisir (step-290d).
...
**[MUST]** OpenTelemetry : un span par étape du pipeline, nommé de façon stable `<domaine>.<étape>` — `pipeline.e164`, `pipeline.opt_out`, `connector.submit`, `dlr.correlate`. Le domaine est l'**étape métier**, jamais le binaire qui l'exécute (`dlr.correlate`, pas `modlrrouter.dlr`) : un lecteur suit un message entre services sans connaître le déploiement. Attributs = identifiants et décisions, jamais le corps ni un secret.

**[MUST]** Un échec se marque sur le span via `observability.RecordSpanError`, **jamais** `SetStatus(codes.Error, err.Error())`. Un span sort du process : il est exporté, stocké, indexé, affiché — c'est la même frontière que `grpcerr` protège côté gRPC. Le statut porte donc toujours le **code plat** (§11.3) ; le texte brut n'est conservé en event `exception` que si l'erreur porte un code connu (ce texte est alors le nôtre). Une erreur non classifiée devient `internal` : ce qu'elle transportait reste dans le process. C'est une barrière d'invariant (a), pas un confort — il suffit qu'une erreur enrobe une valeur contenant le corps.

**[MUST]** Échantillonnage 100 % sur erreur/rejet/timeout. L'échantillonnage de tête décide au **démarrage** du span, avant tout résultat : un simple ratio écarte donc précisément les traces qu'un exploitant veut. `observability.ErrorBiasedSampler` (le span est *enregistré* au lieu d'être *jeté*) et `observability.ErrorBiased` (le processeur n'exporte que les spans échantillonnés ou en erreur, en re-tamponnant ces derniers comme échantillonnés) forment la paire ; l'un sans l'autre ne fait rien. Le re-tamponnage n'est pas cosmétique : le `BatchSpanProcessor` **jette** tout span dont le contexte n'est pas marqué échantillonné, et il le fait dans `enqueue`, pas dans `OnEnd` — un test qui enveloppe un simple `SpanRecorder` ne le verra jamais. Tout span de service doit porter un statut d'erreur quand il échoue (`RecordSpanError` sur le retour nommé) : un parent non marqué est écarté par le ratio et laisse ses enfants **orphelins**, avec un `parent_span_id` qui ne résout vers rien.

Deux coûts assumés, à connaître avant de baisser le ratio. **Tous** les spans sont désormais enregistrés : mesuré à +247 ns et **+953 o par span** (≈ +84 Mo/s d'allocation à 8 000 msg/s), donc baisser `TRACES_SAMPLER_ARG` économise de la bande passante et du stockage, **pas** du CPU ni de la pression GC. Et un span en erreur d'une trace non échantillonnée part sans ses ancêtres hors process — une trace partielle d'un échec vaut mieux que rien, mais un tableau de bord le montrera comme incomplet.
...
| PostgreSQL (credentials de bind) | bind SMPP | **fail-closed, sans aucun cache** : chaque bind lit le plan de contrôle ; une panne rend `ESME_RSYSERR` — **jamais** `ESME_RINVPASWD` sur un sujet non déjà verrouillé, y compris pour un `system_id` inconnu — et elle **ne compte jamais** comme un échec d'auth, donc elle n'arme pas l'anti-brute-force. Symétriquement, un `system_id` qui n'est **pas de l'UTF-8 valide** est refusé *avant* la requête, en `ESME_RINVPASWD` et **compté** : sinon PostgreSQL répondrait `22021` et la panne simulée serait déclenchable par le client, donc exempte du compteur. `/readyz` retire le pod du load balancer | il n'existe aucun cache de credential et il ne doit pas en exister : admettre un ESME dont le statut de compte, l'état du canal et le type de bind n'ont pas pu être lus, c'est admettre une session non authentifiée. `ESME_RINVPASWD` dirait à l'ESME que son secret est faux et le ferait renoncer, là où une panne est transitoire et appelle un retour. Toutes les répliques partageant la même base, le retrait du LB est simultané : la dégradation devient une indisponibilité, et c'est un choix assumé — un pod qui n'authentifie personne n'a rien à offrir. Le « ne compte jamais » a dû être **corrigé** pour devenir vrai : la panne alimentait le throttle anti-brute-force, qui refuse en `ESME_RINVPASWD` au-delà du seuil, si bien qu'au 6ᵉ bind la panne se déguisait en mot de passe faux. La spec ne compte que les *échecs d'auth* (§6.3) et un plan de contrôle muet n'a jugé aucun identifiant. **Ce n'est pas un absolu sur le code rendu** : le throttle est consulté *avant* l'authentification et son compteur est le max de (`system_id`, IP source), donc un verrou acquis **avant** la panne — ou par un voisin derrière le même NAT — continue de répondre `ESME_RINVPASWD` pendant toute sa durée (step-260c) |
| PostgreSQL (clés API) | auth REST | **fail-closed, sans aucun cache** : chaque requête authentifiée lit le plan de contrôle ; une panne rend **500** `internal_error` — **jamais** 401, y compris pour une clé inconnue — et `/readyz` retire le pod du load balancer | même raison que la ligne ci-dessus, plus une qui lui est propre : un 401 affirme que la clé présentée est invalide, et envoie l'intégrateur faire tourner ses identifiants au milieu d'une panne dont il n'est pas la cause. Cette branche n'avait **aucun** test, pas même unitaire, avant step-260c |
...
| Redis (routage L0, numéro exact) | résolution de route | **fail-closed en rejeu** : erreur non codée → offset non commité → redélivrance. **Deux exceptions, et deux seulement** : une valeur illisible et une clé du **mauvais type** (`WRONGTYPE`) sont traitées comme un miss et guéries depuis la table durable | un numéro exact injoignable ne doit pas se dégrader en routage par défaut, qui enverrait sur le mauvais opérateur. Les deux exceptions vont dans l'autre sens : remonter l'erreur renverrait le message sur la **même clé** à chaque redélivrance, et une clé écrite par quelqu'un d'autre ne porte pas notre TTL, donc rien ne l'expire — la partition reste figée jusqu'à un `DEL` manuel (step-290d). Tout autre code RESP (`LOADING`, `READONLY`, `OOM`…) reste une faute, sous test |
| PostgreSQL (routage L0, numéro exact) | résolution de route | **fail-closed en rejeu** : le code plateforme que `translate` pose est **délibérément retiré** sur cette voie, sinon l'erreur serait codée et `router.handle` enterrerait le message (CDR `rejected`, offset commité) au lieu de le redélivrer | `exactroute:{msisdn}` est un cache read-through : depuis step-250e, un possible-hit du Bloom que le cache n'a pas interroge la table durable. Même raison que la ligne Redis ci-dessus — un numéro exact injoignable ne doit pas se dégrader en routage par défaut. Le défaut a été trouvé par le test de chaos, pas par le test unitaire, dont le faux renvoyait un `errors.New` nu : la forme d'une panne, pas son contrat (step-250e) |
| PostgreSQL (snapshots de config) | hot reload du routeur | **asymétrique**. Au **boot**, dépendance dure en deux temps distincts : `postgres.NewPool` pingue à chaud **sans réessayer** (le process sort — CrashLoopBackOff), puis `loadWithRetry` réessaie **sans plafond d'essais** (le pod ne devient jamais ready) ; Redis, lui, passe par `loadWithRetry` dès l'ouverture. **En service**, **dégradé masqué** : un rebuild qui échoue est journalisé puis jeté, et le dernier snapshot bâti continue de servir — ni vidé, ni `nil`, et le pod ne meurt pas | un routeur sans routes ne peut rien router, d'où la dureté au boot ; mais un routeur qui EN A vaut mieux avec des routes périmées qu'avec aucune. Le prix est l'invisibilité : `/readyz` ne regarde que Kafka, aucune métrique d'**échec** de rebuild n'existe, et la seule trace est une ligne de log `Error`. La seule instrumentation voisine, `bloom_last_reload_timestamp_seconds{filter}`, ne couvre que les deux Bloom **et se pose en milieu de closure** : elle avance donc même quand le rebuild échoue plus loin, et affiche « frais » sur un rebuild à moitié raté. Deux aggravants mesurés en step-260c : un échec **en cours** de rebuild laisse les étapes déjà appliquées en place (routes neuves, Bloom / opt-out / scripts / facturation / contenu périmés), et **rien ne rejoue de soi-même** — la réparation attend la prochaine invalidation (correctif fiché en step-395) |
| Redis (jeton d'annulation) | `cancel_sm` | **asymétrique** : fail-closed côté SMPP (refuse l'annulation, `ESME_RSYSERR` — une faute, pas un `ESME_RCANCELFAIL` — et **aucune** ligne CDR) ; fail-open côté pool, en **deux** sites : avant `submit_sm` il journalise et **envoie**, sur la branche d'expiration max-age il journalise et **dead-letter en `delivery_expired`** | une annulation est elle-même best-effort ; en cas de doute on refuse d'annuler, jamais de retenir la livraison (ADR-0013). Sur la branche d'expiration, une annulation concurrente est donc mésenregistrée comme périmée — le message ne partait de toute façon plus (step-250d) |
| PostgreSQL (autorité durable du solde) | crédit MT | **fail-closed avec rejeu** : la réserve refuse et le débit spéculatif du cache est compensé ; le code plateforme que `translate` pose ne **survit pas** à la frontière gRPC, donc le routeur voit une faute transitoire → offset non commité → redélivrance | le cache seul ne prouve rien ; et une panne ne doit jamais se déguiser en refus de fonds, seul cas que le routeur rejette définitivement (step-260b) |
