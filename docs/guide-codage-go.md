...
| PostgreSQL (credentials de bind) | bind SMPP | **fail-closed, sans aucun cache** : chaque bind lit le plan de contrôle ; une panne rend `ESME_RSYSERR` — **jamais** `ESME_RINVPASWD`, y compris pour un `system_id` inconnu et **quel que soit le nombre de tentatives** — et `/readyz` retire le pod du load balancer | il n'existe aucun cache de credential et il ne doit pas en exister : admettre un ESME dont le statut de compte, l'état du canal et le type de bind n'ont pas pu être lus, c'est admettre une session non authentifiée. `ESME_RINVPASWD` dirait à l'ESME que son secret est faux et le ferait renoncer, là où une panne est transitoire et appelle un retour. Toutes les répliques partageant la même base, le retrait du LB est simultané : la dégradation devient une indisponibilité, et c'est un choix assumé — un pod qui n'authentifie personne n'a rien à offrir. Le « quel que soit le nombre de tentatives » a dû être **corrigé** pour devenir vrai : la panne alimentait le throttle anti-brute-force, qui bascule en `ESME_RINVPASWD` au-delà du seuil ; elle ne le fait plus, la spec ne comptant que les *échecs d'auth* (§6.3) et un plan de contrôle muet n'ayant jugé aucun identifiant (step-260c) |
