...
- **Un abonné peut donc recevoir deux fois le même SMS**, et c'est assumé plutôt que subi (ADR-0012). La cause résiduelle est unique : un crash entre le `submit_sm` déjà parti sur le fil et l'accusé du produce Kafka qui enregistre son issue. Aucune conception ne la supprime — un `submit_sm` n'est transactionnel avec aucun magasin — mais elle est **bornée** : au plus **~250 messages par partition et par crash de pod** (`KAFKA_FETCH_MAX_PARTITION_BYTES`, 56 KiB **compressés** — le nombre dépend du taux de compression, mesuré 2,81× sur des records typiques), soit moins d'une demi-seconde de trafic par partition à la cible de débit. Les deux mitigations ci-dessus ne couvrent pas ce cas : les clés d'idempotence client valent à la frontière REST d'ingestion, l'idempotence de facturation protège le solde et non le combiné.
