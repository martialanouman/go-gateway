...
// A pool wired without a producer must refuse to START, not fail on its first send.
//
// The earlier fix here kept the no-op default and made it reject the outcome topic, so a miswired pool
// would "fail loudly on its first send". That is the wrong moment: the publish happens AFTER the
// submit_sm. Such a pool really sends the SMS, the publish is refused, the offset does not commit, the
// record is redelivered — and the same SMS goes out again, in a loop, until someone intervenes. The
// wiring mistake was not made visible; it was turned into a permanent duplicate-delivery incident.
//
// A wiring error has to surface before the process can do anything irreversible (step-201c D8).
func TestNewPanicsWithoutAProducer(t *testing.T) {
	defer func() {
		r := recover()
		if r == nil {
			t.Fatal("New() did not panic: a pool that sends SMS without recording them would start")
		}
		if msg, _ := r.(string); !strings.Contains(msg, "Producer") {
			t.Errorf("panic = %v, want it to name the missing Producer", r)
		}
	}()

	// No Producer, deliberately: that absence is the subject of this test.
	connectorpool.New(connectorpool.Deps{
